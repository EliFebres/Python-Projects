...
stock_data = {  'Returns':stock_yrly_return , 'Standard Deviation': stock_ann_sd, 
...
def optimize_portfolio_sharpe_ratio(tickers):
    ...
    data = {'Returns':all_pf_returns , 'Standard Deviation': all_pf_std, 'Sharpe Ratio': all_pf_sharpe}
    ...
    plt.scatter(y=portfolios['Returns'],x=portfolios['Standard Deviation'], marker='*', s=10, alpha=0.3)
    plt.scatter(y=optimal_risk_port['Returns'], x=optimal_risk_port['Standard Deviation'], color='r', marker='*', s=400, label='Max Sharpe Ratio')
    plt.scatter(y=stock_data['Returns'], x=stock_data['Standard Deviation'], color='b', marker='*', s=400, label='S&P 500 Portfolio')
...
def optimize_portfolio_sortino(tickers):
    ...
    plt.scatter(y=stock_data['Returns'], x=stock_data['Downside Deviation'], color='b', marker='*', s=400, label='S&P 500 Portfolio')
...
def optimize_max_drawdown(tickers):
    ...
    # plt.scatter(y=best_calmar_stock_comp['Returns'], x=best_calmar_stock_comp['Maximum Drawdown'], color='g', marker='*', s=400, label='Out-Of-Sample Portfolio')
    plt.scatter(y=stock_data['Returns'], x=stock_data['Maximum Drawdown'], color='b', marker='*', s=400, label='S&P 500 Portfolio')
